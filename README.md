...
This plugin is a successor of the TeslaConnect Plugin created by Marius H. - see marius-hh/LoxBerry-Plugin-TeslaConnect. Marius H. has decided to stop working for this plugin. I (Jan W.) have accepted the challenge, added support for the new API and made some improvements within the UI to send test queries. The new version got a new name (and starts with version 0.1.1), because it is not allowed to change the name of the author of a Loxberry plugin after the initial release. 
...
The plugin uses the utilities 'tesla-control', 'tesla-keygen', and 'tesla-blescan'. For Raspberry PI 64-bit OS (e.g. Bookworm or Trixie) the plugin contains binary files for these three command line utilities. For all other platforms, you have to follow the instructions provided by Telsa to install 'go' and compile and build the utilities. You may choose to compile and build the utilities on a Raspberry PI if you have security concerns or if Tesla has provided a new version of the SDK.

This plugin shows a list of all commands that are available for your vehicles and powerwalls and you can execute test queries for all commands via gui to know more about each command and the responses.
...
## Setup of plugin
The settings page supports two ways to populate `~/config/plugins/teslacmd/sessiondata.json`: with your devices.

### Tesla account
You can provide tokens for your Tesla account and all devices that are linked to your account will appear in the list. You need to get a refresh and an access token via external utilities shown on the settings page.

### Local BLE scan
Instead of using your Tesla account, you can scan for vehicles nearby, select your vehicle, enter the VIN, and set a nickname.
...
`http://<user>:<pass>@192.168.1.1/admin/plugins/teslacmd/send.php?action=body_controller_state&vin=LRW31234567890123`
### Wake up vehicle
...
# build tesla-blescan utility - may take a while
cd ../tesla-blescan
go build -ldflags "-X 'main.version=$VCVERSION' -X 'main.hwinfo=$HWINFO' -X 'main.hwarch=$HWARCH' -X 'main.today=$TODAY'"  ./...
...
7. Copy binary files (Raspberry PI, 64-bit) to local local bin directory as root user 
   and add rights to 'tesla-control' and 'tesla-blescan' executable to allow BLE commands
...
mv ./vehicle-command/cmd/tesla-blescan/tesla-scan /usr/local/bin/
setcap 'cap_net_admin=eip' /usr/local/bin/tesla-blescan
exit
...
12. (optional) Create a new private and public key pair. You should use the VIN of your car as the name of the key.
...
# ERROR - key is missing in keyring - use 'keyctl show' to display keys in keyring
...
# ERROR - no rights for user to BLE device
